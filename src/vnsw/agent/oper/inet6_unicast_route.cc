...
void Inet6UnicastAgentRouteTable::AddDropRoute(const string &vm_vrf,
                                               const Ip6Address &addr,
                                               uint8_t plen,
                                               const string &vn_name,
    ...
    Agent *agent = Agent::GetInstance();
    DBRequest req(DBRequest::DB_ENTRY_ADD_CHANGE);
    req.key.reset(new Inet6UnicastRouteKey(agent->local_peer(), vm_vrf,
                                           GetIp6SubnetAddress(addr, plen),
                                           plen));
